      ...
      schemas: {
        ...
        Product: {
          ...
          properties: {
            ...
            sizes: { type: 'string', description: 'JSON array of variants, e.g. [{"label":"1 kq","price":4.5},{"label":"5 kq","price":21}]. `label` may also be a per-language object for named variants (flavours): {"label":{"az":"Kəklikotulu","en":"Thyme","ru":"С чабрецом","tr":"Kekikli"},"price":5,"image":"/uploads/x.jpg"} — `image` is an optional variant photo. Empty [] means no variants; `price` then applies.' },
              ...
              'multipart/form-data': {
                schema: {
                  ...
                  properties: {
                    ...
                    carbs: { type: 'number' },
                    allergens: { type: 'string', description: 'JSON array string' },
                    sizes: { type: 'string', description: 'JSON array of {label, price, image?} variants (optional). label: string or {az,en,ru,tr} object.' },
                    is_available: { type: 'integer', enum: [0, 1] },
                    is_featured: { type: 'integer', enum: [0, 1] },
                    is_vegetarian: { type: 'integer', enum: [0, 1] },
                    is_vegan: { type: 'integer', enum: [0, 1] },
